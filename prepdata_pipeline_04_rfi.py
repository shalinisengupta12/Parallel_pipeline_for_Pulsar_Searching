...
def rfi_find(pointing_name, beam_number):
  ...
  #  current_filterbank = '/home/psr/ssengupt/SHALINI_HTRU_North_lowlat/data_files/Test_poitings/' + pointing_name +'/filterbank_files/' +  'beam%s/' %str(beam_number) + pointing_name + '_%s_' %str(beam_number) + '2power.fil'
    ...
    cmds = 'rfifind -ncpus 24 -filterbank -rfips -time 15.0 -zerodm -zapchan 56:58,85:148,415:447,226:229,292 -o %s %s' %(os.path.basename(current_filterbank)[:-9], current_filterbank)
    log = subprocess.check_output(cmds, shell=True)
    print log
    cmds = 'rfifind -ncpus 24 -filterbank -rfips -time 15.0 -zerodm -zapchan 56:58,85:148,415:447,226:229,292 -o %s %s' %(os.path.basename(current_filterbank2)[:-4], current_filterbank2)
    log = subprocess.check_output(cmds, shell=True)
    print log
...
def rfi_find_segmented(pointing_name, beam_number, segment_number, part_number):
    ...
    if segment_number==2:
        if part_number==0:
            ...
            cmds = 'rfifind -ncpus 24 -filterbank -rfips -time 10 -zerodm -intfrac 0.4 -zapchan 56:58,85:148,415:447,226:229,292 -o %s %s' %(os.path.basename(current_filterbank)[:-4], current_filterbank)
            log = subprocess.check_output(cmds, shell=True)
            print log
        else:
            current_filterbank = '/home/psr/ssengupt/SHALINI_HTRU_North_lowlat/data_files/Test_poitings/' + pointing_name +'/filterbank_files/' +  'beam%s/' %str(beam_number) + pointing_name + '_beam_' + str(beam_number) + '_segment_By_' + str(segment_number) + '_part_' + str(part_number) + '.fil'
    ...
    if segment_number==4:
        if part_number==0:
            current_filterbank = '/home/psr/ssengupt/SHALINI_HTRU_North_lowlat/data_files/Test_poitings/' + pointing_name +'/filterbank_files/' +  'beam%s/' %str(beam_number) + pointing_name + '_beam_' + str(beam_number) + '_segment_By_' + str(segment_number) + '_part_' + str(part_number) + '.fil'
            cmds = 'rfifind -ncpus 24 -filterbank -rfips -time 5 -zerodm -intfrac 0.5 -zapchan 56:58,85:145,415:447,226:229,292 -o %s %s' %(os.path.basename(current_filterbank)[:-4], current_filterbank)
            log = subprocess.check_output(cmds, shell=True)
            print log
        elif part_number==1:
            current_filterbank = '/home/psr/ssengupt/SHALINI_HTRU_North_lowlat/data_files/Test_poitings/' + pointing_name +'/filterbank_files/' +  'beam%s/' %str(beam_number) + pointing_name + '_beam_' + str(beam_number) + '_segment_By_' + str(segment_number) + '_part_' + str(part_number) + '.fil'
